...
class SnapshotsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create snapshot" do
    assert_difference("Snapshot.count") do
      post snapshots_url, params: { snapshot: { color: @snapshot.color, title: @snapshot.title } }
    end
  ...
  test "should update snapshot" do
    patch snapshot_url(@snapshot), params: { snapshot: { color: @snapshot.color, title: @snapshot.title } }
